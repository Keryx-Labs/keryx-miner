...
static SMOKE_SPECS: [&keryx_miner::models::ModelSpec; 1] =
    [&keryx_miner::models::QWEN3_8B_ABLITERATED];
...
fn main() -> Result<()> {
    ...
    if !keryx_miner::llama_engine::library_available() {
        bail!("compatible keryx-llama shared library not found next to this executable");
    }
...
fn run_plain(gguf: &str, gpu: usize, cycles: usize) -> Result<()> {
    for cycle in 1..=cycles {
        keryx_miner::llama_engine::ensure_loaded(&gguf, gpu)
            .with_context(|| format!("load cycle {cycle}/{cycles}"))?;
...
fn run_pom_swap(gguf: &str, gpu: usize, cycles: usize) -> Result<()> {
    ...
    for cycle in 1..=cycles {
        ...
        keryx_miner::pom_gpu::uninstall(device_id);
        keryx_miner::llama_engine::unload("smoke_pom_swap");
        std::thread::sleep(Duration::from_millis(250));
    }
