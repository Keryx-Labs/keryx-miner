...
///
/// Keryx model IDs are the digest inside the pinned CIDv0, not a SHA-256 of the
/// raw file bytes. The legacy Kubo importer uses 256 KiB DAG-PB leaves and a
/// balanced layout with 174 links per node.
pub fn unixfs_v0_digest_file(
    path: &Path,
...
mod tests {
    ...
    fn unixfs_fixture_matches_pinned_model_id() {
